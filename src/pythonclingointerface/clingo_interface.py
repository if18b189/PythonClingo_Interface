...
class ClingProblem:
    def __init__(self,tmpFile=None, autoexecute=True):
        self.solution:ClingoSolutions
        if(tmpFile):
            self.path=tmpFile
        else:
            self.fd, self.path = tempfile.mkstemp(suffix='.txt', prefix='clingoInterfaceTemp_', dir=os.getcwd(), text=True)
    ...
    def executeClingoCode(self, code):
        if code is not None:
            with tempfile.TemporaryFile() as tempf:  # reading into a temporary file prevents issues with bigger input values
                ...
                proc.wait()
                tempf.seek(0)
                self.solution=ClingoSolutions(tempf.read())
        else:
